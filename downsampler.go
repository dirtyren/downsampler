...
func (vm *VMClient) DeleteSeries(metric map[string]string, start, end time.Time) error {
	// Skip deletion for internal VM metrics (they typically start with "vm_" or "flag")
	metricName := metric["__name__"]
	if strings.HasPrefix(metricName, "vm_") || strings.HasPrefix(metricName, "flag") {
		log.Printf("Skipping deletion of internal metric: %s", metricName)
		return nil
	}
	...
	for currentStart.Before(end) {
		...
		deleteURL := fmt.Sprintf("%s/api/v1/admin/tsdb/delete_series", vm.baseURL)
		...
		req, err := http.NewRequest("POST", deleteURL, strings.NewReader(params.Encode()))
		if err != nil {
		...
		// If we get 400 or 404, the endpoint might not be available
		if resp.StatusCode == http.StatusBadRequest || resp.StatusCode == http.StatusNotFound {
			return fmt.Errorf("delete endpoint not available or disabled. Response: %s. You may need to enable -retentionPeriod flag in VictoriaMetrics or run with admin privileges", string(body))
		}
	...
	if deletedChunks > 0 {
		log.Printf("Successfully deleted data for %s in %d chunks", vm.buildSelector(metric), deletedChunks)
	}
